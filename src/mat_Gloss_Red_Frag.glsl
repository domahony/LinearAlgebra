...
{
	mat3 obj2view = to_mat3(GL_MVM);
	mat3 view2obj = to_mat3(GL_MVM_I);
...
void shade_view(vec3 co, out vec3 view)
{
	...
	view = (GL_PM[3][3] == 0.0)? normalize(co): vec3(0.0, 0.0, -1.0);
}
...
void test_shadowbuf(vec3 rco, sampler2DShadow shadowmap, mat4 shadowpersmat, float shadowbias, float inp, out float result)
{
	if(inp <= 0.0) {
		...
		if (co.w > 0.0 && co.x > 0.0 && co.x/co.w < 1.0 && co.y > 0.0 && co.y/co.w < 1.0)
			...
			result = texture(shadowmap, vec3(co));
		else
			result = 1.0;
	}
}
...
void shade_mist_factor(vec3 co, float miststa, float mistdist, float misttype, float misi, out float outfac)
{
	...
	zcor = (GL_PM[3][3] == 0.0)? length(co): -co[2];
...
void node_bsdf_diffuse(vec4 color, float roughness, vec3 N, out vec4 result)
{
	...
	for(int i = 0; i < NUM_LIGHTS; i++) {
		vec3 light_position = LIGHT[i].position.xyz;
		vec3 light_diffuse = LIGHT[i].diffuse.rgb;
...
void node_bsdf_glossy(vec4 color, float roughness, vec3 N, out vec4 result)
{
	...
	for(int i = 0; i < NUM_LIGHTS; i++) {
		vec3 light_position = LIGHT[i].position.xyz;
		vec3 H = LIGHT[i].halfVector.xyz;
		vec3 light_diffuse = LIGHT[i].diffuse.rgb;
		vec3 light_specular = LIGHT[i].specular.rgb;
...
//const vec4 cons1 = vec4(0.800000, 0.064000, 0.064000, 1.000000);
const float cons2 = float(0.000000);
in vec3 varnormal;
in vec3 color;
//const vec4 cons5 = vec4(0.800000, 0.115253, 0.166235, 1.000000);
const float cons6 = float(0.000000);
const float cons9 = float(0.700000);
const vec4 cons14 = vec4(0.000000, 0.000000, 0.000000, 0.000000);
const float cons15 = float(0.000000);
out vec4 GL_FRAG_COLOR; 
...
void main(void)
{
	...
	vec4 cons1 = vec4(color.x, color.y, color.z, 1.0);	
	vec4 cons5 = cons1; 
	
	node_bsdf_diffuse(cons1, cons2, facingnormal, tmp4);
	node_bsdf_glossy(cons5, cons6, facingnormal, tmp8);
	node_mix_shader(cons9, tmp4, tmp8, tmp12);
	node_output_material(tmp12, cons14, cons15, tmp16);
	linearrgb_to_srgb(tmp16, tmp18);
