...
App2::App2(const int& width, const int& height) : App(), display(width, height), physics(),
		...
		light(glm::normalize(glm::vec3(-1,-1,0)), glm::vec3(1,1,1), 0.1), active(0), middle_active(false)
{
...
_init()
{
	...
	boost::shared_ptr<AppObject> plane(new AppObject(glm::translate(glm::mat4(1.), glm::vec3(0, 0, 0)), program, groundplane_data, m1, 1));
	//boost::shared_ptr<AppObject> obj2(new AppObject(glm::translate(glm::mat4(1.), glm::vec3(3, 3, 10)), program, data, m1));
	boost::shared_ptr<AppObject> obj(new AppObject(glm::translate(glm::mat4(1.), glm::vec3(0, 5, 0)), program, data, m1, 1));
	boost::shared_ptr<AppObject> die4_obj(new AppObject(glm::translate(glm::mat4(1.), glm::vec3(2, 5, 0)), program, die4_data, m1, 1));
	boost::shared_ptr<AppObject> die4_obj2(new AppObject(glm::translate(glm::mat4(1.), glm::vec3(0, 6, 5)), program, die4_data, m1, 1));
	boost::shared_ptr<AppObject> die4_obj3(new AppObject(glm::translate(glm::mat4(1.), glm::vec3(3, 3, 3)), program, die4_data, m1, 1));
...
motion(const SDL_MouseMotionEvent& m)
{
	if (middle_active) {
		...
		camera.move(glm::vec3(-m.xrel, -m.yrel, 0), dist);
		return true;
